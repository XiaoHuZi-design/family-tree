...
const { checkLogin, checkFamilyMembership, checkFamilyRole } = require('../../common/utils/permission.js');
const { ROLES, GENERATIONS, ERROR_CODES, PAGE_SIZE, MAX_GENERATIONS } = require('../../common/utils/constants.js');
...
module.exports = {
  ...
  async createFamily(data) {
    ...
    if (!data || typeof data !== 'object') {
      uniCloud.throw({ code: ERROR_CODES.PARAM_ERROR.code, message: 'data 参数必填' });
    }
    ...
    if (!familyName || typeof familyName !== 'string' || familyName.length < 2 || familyName.length > 20) {
      uniCloud.throw({ code: ERROR_CODES.PARAM_ERROR.code, message: 'familyName 必填且长度为2-20字符' });
    }

    if (description && typeof description !== 'string') {
      uniCloud.throw({ code: ERROR_CODES.PARAM_ERROR.code, message: 'description 必须为字符串' });
    }
    ...
    try {
      ...
      uniCloud.throw({ code: e.code || ERROR_CODES.DB_ERROR.code, message: e.message || ERROR_CODES.DB_ERROR.message });
    }
  },
  ...
  async getFamilyTree(familyId) {
    ...
    if (!familyId || typeof familyId !== 'string') {
      uniCloud.throw({ code: ERROR_CODES.PARAM_ERROR.code, message: 'familyId 必填' });
    }
  ...
  async getMemberDetail(memberId) {
    ...
    if (!memberId || typeof memberId !== 'string') {
      uniCloud.throw({ code: ERROR_CODES.PARAM_ERROR.code, message: 'memberId 必填' });
    }

    try {
      const memberRes = await db.collection('familyMember').doc(memberId).get();
      if (!memberRes.data || memberRes.data.length === 0) {
        uniCloud.throw({ code: ERROR_CODES.MEMBER_NOT_FOUND.code, message: ERROR_CODES.MEMBER_NOT_FOUND.message });
      }
  ...
  async addMember(data) {
    ...
    if (!name || typeof name !== 'string') {
      uniCloud.throw({ code: ERROR_CODES.PARAM_ERROR.code, message: 'name 必填' });
    }

    if (!generation || typeof generation !== 'number' || generation < 1 || generation > 4) {
      uniCloud.throw({ code: ERROR_CODES.PARAM_ERROR.code, message: 'generation 必须为1-4' });
    }
  ...
  async updateMember(memberId, data) {
    ...
    try {
        ...
        if (data[field] !== undefined) {
          if (field === 'role') {
            ...
            if (memberInfo.role !== ROLES.OWNER) {
              uniCloud.throw({ code: ERROR_CODES.PERMISSION_DENIED.code, message: '只有族主可修改角色' });
            }
            if (![ROLES.OWNER, ROLES.ADMIN, ROLES.MEMBER].includes(data[field])) {
              uniCloud.throw({ code: ERROR_CODES.PARAM_ERROR.code, message: 'role 值不合法' });
            }
          }
      ...
      if (Object.keys(updates).length === 0) {
        uniCloud.throw({ code: ERROR_CODES.PARAM_ERROR.code, message: '至少更新一个字段' });
      }
  ...
  async searchMembers(familyId, keyword) {
    ...
    if (!keyword || typeof keyword !== 'string') {
      uniCloud.throw({ code: ERROR_CODES.PARAM_ERROR.code, message: 'keyword 必填' });
    }
  ...
  async inviteMember(familyId, phone) {
    ...
    if (!phone || typeof phone !== 'string') {
      uniCloud.throw({ code: ERROR_CODES.PARAM_ERROR.code, message: 'phone 必填' });
    }
    ...
    try {
      ...
      if (!userRes.data || userRes.data.length === 0) {
        uniCloud.throw({ code: ERROR_CODES.USER_NOT_FOUND.code, message: '该手机号未注册' });
      }
      ...
      if (existRes.data && existRes.data.length > 0) {
        uniCloud.throw({ code: ERROR_CODES.ALREADY_IN_FAMILY.code, message: '该用户已是家族成员' });
      }
